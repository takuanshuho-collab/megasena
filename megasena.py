...
Recursos principais:
- Regras: anti-sequência (max_run), paridade (target_even ± tolerance), mesmo final (max_same_ending), soma (min_sum..max_sum), span mínimo (min_span), máximo por grupo de dezenas (max_same_tens).
- Cache local: reduz chamadas à API e reaproveita dados.
- Exportações: CSV de frequências e jogos; Excel (.xlsx) com abas Frequencias (com gráfico + formatação condicional), Jogos, Parametros (com validação), Historico, Metadados.
- Flag --no-chart: suprime geração de PNG do gráfico.
- Flag --export-dir: salva todos os artefatos em subpasta com timestamp.
- Flag --cf-mode: escolhe o modo de formatação condicional (formula | top10) para o Excel.

Dependências: requests, beautifulsoup4, matplotlib (opcional), pandas (engine openpyxl), openpyxl
"""
...
def export_excel(
    ...
    Adiciona gráfico de barras na aba Frequencias e aplica formatação condicional conforme cf_mode.
    """
    import pandas as pd
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            wb = writer.book
            # Gráfico + formatação condicional
            if add_chart:
                ws = wb['Frequencias']
...
def main() -> None:
    ...
    if export_dir:
        ...
        export_excel(
            ...
            draws=fetch.draws, metadata=excel_meta, add_chart=(not args.no_chart), cf_mode=args.cf_mode
        )
    else:
        if args.games_csv:
