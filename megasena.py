...
def export_excel(
    ...
    cf_mode: str = "formula",
    cf_rank: int = 20,
    cf_colors: str | None = None,
) -> str:
    """Exporta um Excel (.xlsx) com abas: Frequencias, Jogos, Parametros, Historico e Metadados.
    Adiciona gráfico de barras na aba Frequencias e aplica formatação condicional conforme cf_mode/cf_rank/cf_colors.
    """
    import pandas as pd
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            if add_chart:
                ...
                color_scale = ColorScaleRule(
                    start_type='min', start_color=c_start,
                    mid_type='percentile', mid_value=50, mid_color=c_mid,
                    end_type='max', end_color=c_end,
                )
                ...
                if cf_mode == 'formula':
                    ws.conditional_formatting.add(
                        freq_range,
                        FormulaRule(formula=[f'RANK(B2,$B$2:$B$61)<={cf_rank}'], fill=top_fill)
                    )
                    ws.conditional_formatting.add(
                        freq_range,
                        FormulaRule(formula=[f'RANK(B2,$B$2:$B$61,1)<={cf_rank}'], fill=bottom_fill)
                    )
                else:
            ...
            # Validação de dados na aba Parametros (coluna B)
            ...
            for row in range(2, ws_params.max_row + 1):
                name = ws_params.cell(row, 1).value
                cell = ws_params.cell(row, 2).coordinate
                if name == 'mix':
                    add_dv_list(cell, ['3-2-1','2-2-2','4-1-1','1-3-2'])
                elif name == 'max_run':
                    add_dv_whole(cell, 1, 3)
                elif name == 'target_even':
                    add_dv_whole(cell, 2, 4)
                elif name == 'tolerance':
                    add_dv_whole(cell, 0, 2)
                elif name == 'max_same_ending':
                    add_dv_whole(cell, 1, 3)
                elif name == 'min_sum':
                    add_dv_whole(cell, 90, 300)
                elif name == 'max_sum':
                    add_dv_whole(cell, 120, 330)
                elif name == 'min_span':
                    add_dv_whole(cell, 20, 35)
                elif name == 'max_same_tens':
                    add_dv_whole(cell, 2, 4)
                elif name == 'seed':
                    add_dv_whole(cell, 0, 999999)
                elif name == 'n_games':
                    add_dv_whole(cell, 1, 20)
...
# Wrapper público para testes

def max_consecutive_run(nums: List[int]) -> int:
    return _max_consecutive_run(nums)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-chart", action="store_true", help="Não gerar gráfico de frequência (PNG)")
    ...
    parser.add_argument("--export-xlsx", type=str, default=None, help="Caminho para salvar Excel (.xlsx)")
    parser.add_argument("--export-dir", type=str, default=None, help="Diretório para salvar artefatos em subpasta com timestamp")
    parser.add_argument("--cf-mode", type=str, choices=["formula","top10"], default="formula", help="Modo de formatação condicional do Excel (formula|top10)")
    parser.add_argument("--cf-rank", type=int, default=20, help="Quantidade de valores a destacar (Top e Bottom)")
...
def main() -> None:
    ...
    # Diretório de exportação com timestamp
    ...
    if export_dir:
        ...
        export_excel(
            freq, games, str(export_dir / "megasena.xlsx"), params,
            draws=fetch.draws, metadata=excel_meta, add_chart=(not args.no_chart),
            cf_mode=args.cf_mode, cf_rank=args.cf_rank, cf_colors=args.cf_colors,
        )
    else:
        if args.games_csv:
        ...
        if args.export_xlsx:
            export_excel(
                freq, games, args.export_xlsx, params,
                draws=fetch.draws, metadata=excel_meta, add_chart=(not args.no_chart),
                cf_mode=args.cf_mode, cf_rank=args.cf_rank, cf_colors=args.cf_colors,
            )
