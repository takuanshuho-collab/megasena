"""
Mega-Sena pipeline com coleta (API/Cache/Scraping), frequência, geração de jogos e exportações.
Inclui: gráfico PNG (opcional), Excel com abas (Frequencias, Jogos, Parametros, Historico, Metadados),
formatação condicional configurável (modo, rank, cores) e validação na aba Parametros.

Flags principais:
- --export-dir: salva artefatos em subpasta com timestamp.
- --export-xlsx: salva Excel em caminho específico.
- --no-chart: não gera PNG do gráfico.
- --cf-mode: modo de CF (formula|top10).
- --cf-rank: quantidade de valores destacados para Top/Bottom (padrão 20).
- --cf-colors: cores personalizadas (start,mid,end,top,bottom) sem '#'.
"""
    ...
    if latest and isinstance(latest, dict) and "numero" in latest:
        ...
        if last_number:
            ...
            if draws:
                ...
                FETCH_SOURCE = "api"; LAST_CONCURSO = last_number
                return FetchResult(draws=draws, history_set=history_set)
    logging.warning("Falhou obter o último concurso na API base.")
    cache = _load_cache()
    if cache:
        FETCH_SOURCE = "cache"; return cache
    logging.info("API indisponível e sem cache válido. Iniciando fallback por scraping (BeautifulSoup)")
    FETCH_SOURCE = "scraping"; return _fallback_scraping()

def _fallback_scraping() -> FetchResult:
    draws: List[List[int]] = []; history_set: Set[Tuple[int, ...]] = set()
    urls = ["https://www.megasena.com/resultados", "https://www.mazusoft.com.br/mega/resultados.php"]
    ...
    for url in urls:
        try:
            ...
            if r.status_code != 200:
                logging.warning(f"HTTP {r.status_code} ao scrapear {url}"); continue
            soup = BeautifulSoup(r.text, "html.parser"); texts = soup.stripped_strings
            buffer: List[int] = []
            for token in texts:
                for match in pattern.findall(token):
                    try: num = int(match)
                    except Exception: continue
                    if 1 <= num <= 60: buffer.append(num)
                if len(buffer) >= 6:
                    nums = buffer[-6:]
                    if len(nums) == 6:
                        tup = tuple(sorted(nums))
                        if tup not in history_set:
                            draws.append(nums); history_set.add(tup)
                    if len(draws) >= 100: break
            if len(draws) >= 100: break
        except requests.exceptions.RequestException as e:
            logging.error(f"Erro de conexão no scraping {url}: {e}"); continue
    logging.info(f"Scraping concluído com {len(draws)} concursos");
    if draws: _save_cache(draws, meta={"source": "scraping"})
    return FetchResult(draws=draws, history_set=history_set)
...
def plot_frequency(freq: Dict[int, int], filename: str = "freq_megasena.png") -> str:
    ...
    xs = list(range(1, 61)); ys = [freq[i] for i in xs]
    plt.figure(figsize=(14, 6)); plt.bar(xs, ys, color="steelblue")
    plt.title("Frequência das dezenas (últimos concursos)"); plt.xlabel("Dezena"); plt.ylabel("Frequência")
    plt.xticks(xs, [f"{i:02d}" for i in xs], rotation=0, fontsize=8); plt.grid(axis="y", linestyle=":", alpha=0.4)
    plt.tight_layout(); plt.savefig(filename, dpi=200); plt.close(); logging.info(f"Gráfico salvo em: {filename}")
    return filename
...
def export_frequency_csv(freq: Dict[int, int], filename: str) -> str:
    ...
    try:
        with open(filename, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f); writer.writerow(["dezena", "frequencia"])
            for d, c in rows: writer.writerow([d, c])
        logging.info(f"CSV de frequência salvo em: {filename}"); return filename
    except Exception as e:
        logging.error(f"Falha ao salvar CSV de frequência: {e}"); return filename
...
def export_games_csv(games: List[List[int]], filename: str, params: dict) -> str:
    ...
    try:
        with open(filename, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            header = ["timestamp","game_index","numbers","mix","max_run","target_even","tolerance","max_same_ending","min_sum","max_sum","min_span","max_same_tens","seed","n_games"]
        ...
        logging.info(f"CSV de jogos salvo em: {filename}"); return filename
    except Exception as e:
        logging.error(f"Falha ao salvar CSV de jogos: {e}"); return filename

def export_excel(freq: Dict[int, int], games: List[List[int]], filename: str, params: dict, draws: List[List[int]] | None = None, metadata: Dict[str, str] | None = None, add_chart: bool = True, cf_mode: str = "formula", cf_rank: int = 20, cf_colors: str | None = None) -> str:
    """Exporta Excel com abas e CF configurável.
    cf_mode: 'formula' usa FormulaRule + RANK; 'top10' usa Rule(type='top10') + DifferentialStyle.
    cf_rank: N de valores para destacar (Top/Bottom).
    cf_colors: cores personalizadas (start,mid,end,top,bottom), sem '#'.
    """
    import pandas as pd
    ...
    if draws and len(draws) > 0:
        ...
        for idx, d in enumerate(draws, start=1):
            ...
            for j, val in enumerate(sorted(d), start=1): row[f"D{j}"] = val
            rows.append(row)
    ...
    # Metadados
    meta = metadata or {}
    base_meta = {"coleta_timestamp": ts, "total_concursos": len(draws or []), "fonte": meta.get("fonte", "desconhecida"), "ultimo_concurso": meta.get("ultimo_concurso", ""), "observacao": meta.get("observacao", "Resultados coletados dos últimos até 100 concursos")}
    df_meta = pd.DataFrame({"campo": list(base_meta.keys()), "valor": list(base_meta.values())})

    # Parse cores
    default_start, default_mid, default_end = 'FFC7CE', 'FFEB84', 'C6EFCE'
    default_top, default_bottom = 'C6EFCE', 'FFC7CE'
    c_start, c_mid, c_end = default_start, default_mid, default_end
    c_top, c_bottom = default_top, default_bottom
    if cf_colors:
        try:
            parts = [x.strip().lstrip('#') for x in cf_colors.split(',') if x.strip()]
            if len(parts) == 5:
                c_start, c_mid, c_end, c_top, c_bottom = parts
        except Exception:
            pass
    ...
    try:
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            if add_chart:
                ...
                chart.add_data(data_ref, titles_from_data=False); chart.set_categories(cats_ref); ws.add_chart(chart, 'E2')
                # Escala
                freq_range = 'B2:B61'
                color_scale = ColorScaleRule(start_type='min', start_color=c_start, mid_type='percentile', mid_value=50, mid_color=c_mid, end_type='max', end_color=c_end)
                ws.conditional_formatting.add(freq_range, color_scale)
                # Top/Bottom
                top_fill = PatternFill(start_color=c_top, end_color=c_top, fill_type='solid')
                bottom_fill = PatternFill(start_color=c_bottom, end_color=c_bottom, fill_type='solid')
                if cf_mode == 'formula':
                    ws.conditional_formatting.add(freq_range, FormulaRule(formula=[f'RANK(B2,$B$2:$B$61)<={cf_rank}'], fill=top_fill))
                    ws.conditional_formatting.add(freq_range, FormulaRule(formula=[f'RANK(B2,$B$2:$B$61,1)<={cf_rank}'], fill=bottom_fill))
                else:
                    dxf_top = DifferentialStyle(fill=top_fill)
                    rule_top = Rule(type='top10', rank=cf_rank, percent=False, bottom=False, dxf=dxf_top)
                    ws.conditional_formatting.add(freq_range, rule_top)
                    dxf_bottom = DifferentialStyle(fill=bottom_fill)
                    rule_bottom = Rule(type='top10', rank=cf_rank, percent=False, bottom=True, dxf=dxf_bottom)
                    ws.conditional_formatting.add(freq_range, rule_bottom)
            # Validação de dados
            ws_params = wb['Parametros']
            def add_dv_list(cell, items):
                dv = DataValidation(type='list', formula1='"' + ','.join(items) + '"', allow_blank=True, showDropDown=True); ws_params.add_data_validation(dv); dv.add(cell)
            def add_dv_whole(cell, minv, maxv):
                dv = DataValidation(type='whole', operator='between', formula1=str(minv), formula2=str(maxv), allow_blank=True); ws_params.add_data_validation(dv); dv.add(cell)
        ...
        logging.info(f"Excel salvo em: {filename}"); return filename
    except Exception as e:
        logging.error(f"Falha ao salvar Excel: {e}"); return filename
...
def generate_games(freq: Dict[int, int], history_set: Set[Tuple[int, ...]], n_games: int = 3, seed: int | None = None, mix: Tuple[int, int, int] = (3,2,1), max_run: int = 2, target_even: int = 3, tolerance: int = 0, max_same_ending: int = 2, min_sum: int = 100, max_sum: int = 260, min_span: int = 25, max_same_tens: int = 3) -> List[List[int]]:
    ...
    for gi in range(n_games):
        ...
        while attempts < max_attempts:
            ...
            if not _is_valid_game(candidate, history_set, max_run=max_run, target_even=target_even, tolerance=tolerance, max_same_ending=max_same_ending, min_sum=min_sum, max_sum=max_sum, min_span=min_span, max_same_tens=max_same_tens): continue
...
def _parse_mix(value: str) -> Tuple[int,int,int]:
    ...
    if any(x < 0 for x in t): raise argparse.ArgumentTypeError("mix não pode ter negativos"); return t

def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Gerador de jogos da Mega-Sena com regras, gráfico, cache e exportação")
    parser.add_argument("--mix", type=_parse_mix, default=(3,2,1)); parser.add_argument("--max-run", type=int, default=2)
    parser.add_argument("--target-even", type=int, default=3); parser.add_argument("--tolerance", type=int, default=0)
    parser.add_argument("--max-same-ending", type=int, default=2); parser.add_argument("--min-sum", type=int, default=100)
    parser.add_argument("--max-sum", type=int, default=260); parser.add_argument("--min-span", type=int, default=25)
    parser.add_argument("--max-same-tens", type=int, default=3); parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--n-games", type=int, default=3); parser.add_argument("--no-chart", action="store_true")
    parser.add_argument("--freq-csv", type=str, default=None); parser.add_argument("--games-csv", type=str, default=None)
    parser.add_argument("--export-xlsx", type=str, default=None); parser.add_argument("--export-dir", type=str, default=None)
    parser.add_argument("--cf-mode", type=str, choices=["formula","top10"], default="formula")
    parser.add_argument("--cf-rank", type=int, default=20)
    parser.add_argument("--cf-colors", type=str, default=None, help="Cores CF: start,mid,end,top,bottom (hex, sem #)")
    return parser

def main() -> None:
    parser = build_arg_parser(); args = parser.parse_args()
    logging.info("===== Mega-Sena (pipeline completo) =====")
    fetch = fetch_data(); if not fetch.draws: logging.error("Nenhum concurso coletado. Encerrando."); return
    ...
    if export_dir:
        ...
        export_excel(freq, games, str(export_dir / "megasena.xlsx"), params, draws=fetch.draws, metadata=excel_meta, add_chart=(not args.no_chart), cf_mode=args.cf_mode, cf_rank=args.cf_rank, cf_colors=args.cf_colors)
    else:
        if args.games_csv: export_games_csv(games, args.games_csv, params)
        if args.export_xlsx: export_excel(freq, games, args.export_xlsx, params, draws=fetch.draws, metadata=excel_meta, add_chart=(not args.no_chart), cf_mode=args.cf_mode, cf_rank=args.cf_rank, cf_colors=args.cf_colors)
